export class ExcavationSessionManager {
  constructor(options = {}) {
    ...
    this.maxSnapshots = typeof options.maxSnapshots === "number" ? options.maxSnapshots : 20;
  ...
  addSnapshot(sessionId, excavationResult, label = "default") {
    ...
    const snapshot = {
      ...
      metrics: {
        ...
        domSheets: excavationResult.domSheets ? excavationResult.domSheets.length : 0
      },
  ...
  _detectDrift(session) {
    ...
    const voDelta = Math.abs(latest.metrics.virtualObjects - prev.metrics.virtualObjects);
    const relDelta = Math.abs(latest.metrics.relationships - prev.metrics.relationships);
