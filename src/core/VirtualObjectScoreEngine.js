export class VirtualObjectScoreEngine {
  constructor(options = {}) {
    this.historyWindow = typeof options.historyWindow === "number" ? options.historyWindow : 10;
  ...
  _makeKey(vo) {
    ...
    const sig = typeof vo.signature === "string" ? vo.signature : JSON.stringify(vo.fields || {});
    return `${cat}:${sig}`;
  }
  ...
  _computeStability(hist) {
    ...
    const normalized = Math.max(0, Math.min(1, avgInterval / (24 * 60 * 60 * 1000)));
    return normalized;
  }
  ...
  _computeNovelty(hist) {
    ...
    const normalized = Math.max(0, Math.min(1, ageMs / (7 * 24 * 60 * 60 * 1000)));
    return normalized;
  }
  ...
  _computeReuseHint(vo) {
    ...
    if (vo.category === "dom-tag") {
      return "dom-structure";
    }
    if (vo.category === "struct") {
      return "api-schema";
    }
    return "misc";
  }
